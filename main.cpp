...
///////////////////////////////////global vairables///////////////////////////////////////////////////
static int st=0,ed=23,cursor=0,win_row=30,win_col=150;
        ...
        if ( (stat(file_path.c_str(), &stats) == 0)  &&  dir_list[i]!="." && dir_list[i]!="..")		//stat() returns 0 on successful operation, otherwise returns -1 if unable to get file properties.
   		{
			...
			struct group  *grp = getgrgid(stats.st_gid);
   			if(usr)
   				 cout<<left<<setw(13)<<usr->pw_name<<" ";
   			if(grp)
   				 cout<<left<<setw(13)<<grp->gr_name<<" ";
   			
   			//modified date time
	        dt = *(gmtime(&stats.st_mtime));

	        cout<<left<<setw(2)<<dt.tm_mday
	        <<left<<setw(1)<<"-"
	        <<left<<setw(2)<<dt.tm_mon
	        <<left<<setw(1)<<"-"
	        <<left<<setw(4)<<(dt.tm_year + 1900)
	        <<left<<setw(1)<<" " 
            <<left<<setw(2)<<dt.tm_hour
            <<left<<setw(1)<<":"
            <<left<<setw(2)<<dt.tm_min
            <<left<<setw(1)<<":"
            <<left<<setw(5)<<dt.tm_sec;
            //cout<<setw(4);

   			//size
   			int fsize = stats.st_size;
   			vector<string> format{ "B", "KB", "MB", "GB", "TB" };
    		double dblByte = fsize;
    		int j;
    		for (j = 0; j < 5 && fsize >= 1024; j++, fsize /= 1024)
    	    	dblByte = fsize / 1024.0;
 
   			fsize=dblByte;		//convert to int
   			string op_size= to_string(fsize) + " " + format[j];
   			
	        cout<<left<<setw(16)<<op_size;
...
void leftkey(int &st, int &ed, int &cursor, vector<string> &dir_list, stack<string> &back_stack, stack<string> &fwd_stack, string &path)
{
	if(!back_stack.empty())
	{
		...
		st=0,ed=23,cursor=0;							//update pointers
...
void enterkey(int &st, int &ed, int &cursor, vector<string> &dir_list, stack<string> &back_stack, stack<string> &fwd_stack, string &path)
{
	...
	else if(stat_flag>=0 && !S_ISDIR(fileStat.st_mode))	//cursor points to file
	{
		...
		{
			string cmd="vi";						//set  to "xdg-open" to open files using default software 
...
void copyfile(string para1,string para2)
{
	...
	struct stat fileStat;								//create a stat strict
	char char_array[src.length()+1];					//convert path to char array to pas to stat function
	strcpy(char_array, src.c_str()); 
	int stat_flag = stat(char_array, &fileStat);		//check if stat worked. if flag<0 then error		

	if(stat_flag>=0 && !S_ISDIR(fileStat.st_mode))		//Parameter is a file
	{
		if(para2==".")
		{
			dest=path + "/" + para1;
	    ...
	    chmod(dest.c_str(),
	    (fileStat.st_mode & S_IRUSR)|(fileStat.st_mode & S_IWUSR)|(fileStat.st_mode & S_IXUSR)|
...
bool searchfile(string fname,string para1)				//here para 1 is abs path
{
	...
	for(int i=0 ; i<search_list.size() ; i++)
	{
		...
		{
			if(search_list[i]==fname)					//file/dir name mathced
				return true;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
						...
						//abs path of src and dest
void copydir_fileutil(string oldf,string newf)
{
	struct stat fileStat;								//create a stat strict
	char char_array[oldf.length()+1];					//convert path to char array to pass to stat function
	strcpy(char_array, oldf.c_str());
	int stat_flag = stat(char_array, &fileStat);		//check if stat worked. if flag<0 then error		

	if(stat_flag>=0 && !S_ISDIR(fileStat.st_mode))		//Parameter is a file
	{		//store fd

		int fd1 = open(oldf.c_str(), O_RDONLY, 0);				//bryant ch10
	    int fd2 = open(newf.c_str(), O_WRONLY | O_CREAT, 0);
	    char ch;
	    mode_t mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH ;		//set permissions (byrant PDF page 864)
	    //cout<<endl;
	    while(read(fd1, &ch, 1) != 0)
	    ...
	    if (stat(newf.c_str(), &status) == -1) 
	       return;
	}
	   
}
void copydir_dirutil(string oldd,string newd)
{
	vector<string> namelist=list_files(oldd);

	for(int i=0;i<namelist.size();i++)
	{
		struct stat fileStat;
		string cur = oldd +"/" +  namelist[i];						//current file path
		char char_array[(cur).length()+1];					//convert path to char array to pas to stat function
		strcpy(char_array, cur.c_str()); 
		int stat_flag = stat(char_array, &fileStat);		//check if stat worked. if flag<0 then error

		if(stat_flag>=0 && namelist[i]!="." && namelist[i]!=".." && !S_ISDIR(fileStat.st_mode)   )		//Parameter is a file
		{
			//cout<<"afsd";
			string para1=(oldd + "/" + namelist[i]);
			string para2=(newd + "/" + namelist[i]);
			copydir_fileutil(para1,para2);
		}
		else if(stat_flag>=0  && namelist[i]!="." && namelist[i]!=".." && S_ISDIR(fileStat.st_mode) )
		{
			string para2=(newd + "/" + namelist[i]);
			
			string para1=(oldd + "/" + namelist[i]);
			mode_t mode = S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH |S_IWOTH | S_IXOTH;
			int fd= mkdir(para2.c_str(), mode);

			copydir_dirutil( oldd, newd);	

		}		

	}
}
void copydir(string dirname,string dirpath)
{
	string pwd=get_current_dir_name();
	dirpath.erase(0,1);
	string src = path + "/" + dirname;
	string dest = pwd + "/" + dirpath + "/" + dirname;		//after creating directory we copy in this path

	mode_t mode = S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH |S_IWOTH | S_IXOTH;		//set permissions (byrant PDF page 864)
	int fd;			//store fd
	fd = mkdir(dest.c_str(), mode);
	//exit(0);
	copydir_dirutil(src,dest);
    ...
    while(1)
    {
		...
		else if(key1==58)				// colon key - command mode
		{
			...
			while(1)
			{
				...
				/*  THIS CONDITION OVERSHADOWS 'A' AND 'B'
